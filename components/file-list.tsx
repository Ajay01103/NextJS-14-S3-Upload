...
export const FileList = ({ files, loading, loadingFileNames, onDelete }: Props) => {
  return (
...
const FileComponent = ({
  ...
  return (
    <div
      key={file.id}
      className={cn("masonry mb-4", loading && "animate-pulse")}
    >
      <a
        href={file.url}
      ...
      <div className="mt-2 flex flex-col justify-between text-base font-medium text-gray-900">
        ...
        <div className={"flex w-full justify-between"}>
          <span className={"text-gray-500 text-xs block"}>{formatDateString(file.modified)}</span>
          {onDelete && (
            <button
              onClick={onDelete}
